...
def test_payables_aging():
    with Fake1CServer() as srv:
        out = make(srv).payables_aging(as_of="2026-07-31T00:00:00")
...
def test_reconciliation_act_balances():
    with Fake1CServer() as srv:
        out = make(srv).reconciliation_act(GUID_ROMASHKA)
...
def test_reconciliation_act_unknown_counterparty():
    with Fake1CServer() as srv:
        out = make(srv).reconciliation_act("00000000-0000-0000-0000-000000000000")
...
def test_cash_flow_states_missing_opening_balance():
    ...
    with Fake1CServer() as srv:
        assert "Остаток на счёте не показан" in make(srv).cash_flow()


def test_cash_flow_respects_period():
    with Fake1CServer() as srv:
        out = make(srv).cash_flow(date_from="2026-07-01T00:00:00",
                                  date_to="2026-07-31T23:59:59")
