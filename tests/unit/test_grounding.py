...
def _reports():
    ...
    with Fake1CServer() as srv:
        ...
        yield "abc", str(a.abc_analysis("counterparty")), (
            ...
            "210 000.00 руб. (78.5%) и «ООО «Василёк»» 57 500.00 руб. (21.5%). "
            "Всего 267 500.00 руб.")
        yield "receivables", str(a.receivables_aging(as_of="2026-07-31T00:00:00")), (
            "Нам должны 186 000.00 руб. «ООО «Ромашка»» — 132 000.00 руб. "
            "(№РТ-0001 от 03.07.2026 на 120 000.00 и №РТ-0005 от 25.06.2026 "
            "на 12 000.00), «ООО «Василёк»» — 54 000.00 руб.")
        ...
        yield "cash_flow", str(a.cash_flow()), (
            "За всё время поступило 155 000.00 руб., списано 200 000.00 руб., "
            "чистый поток минус 45 000.00 руб.")
        yield "pnl", str(a.pnl_report()), (
            "Выручка 267 500.00 руб., себестоимость 178 333.33 руб., "
            "валовая прибыль 89 166.67 руб. Это валовая прибыль: расходы по "
        ...
        yield "dynamics", str(a.sales_dynamics()), (
            "Продажи растут: май 77 500.00, июнь 82 500.00, июль 107 500.00 руб. "
            "Средний чек в июле 53 750.00 руб. при 2 отгрузках.")
...
def test_single_wrong_digit_in_a_report_is_caught():
    ...
    import re
    for name, report, answer in _reports():
        m = re.search(r"\d[\d  ]{2,}\.\d{2}", answer)
        assert m, f"{name}: в пересказе нет суммы, тест бессмысленен"
        # Меняем старшую цифру суммы — сумма остаётся правдоподобной.
        digit = m.group(0)[0]
        mutated = str((int(digit) + 1) % 10) + m.group(0)[1:]
        broken = answer[:m.start()] + mutated + answer[m.end():]
