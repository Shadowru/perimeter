...
- Прод-раскатка лендинга (RU prod 104.171.139.52, по /etc/agents-shared.md): код в `/opt/perimeter` (деплой = git push → git pull на проде), статика из `docs/` через file_server в общем Caddy. Основной домен: **https://впериметре.рф/** (`xn--b1agaanysjdt.xn--p1ai`) — живой, сертификаты выпущены (вкл. www-редирект); `perimeter.extra.moscow` — постоянный 301-редирект на основной домен.
